...
public class MyPushMessageReceiver extends FrontiaPushMessageReceiver {
    ...
    public void onBind(Context context, int errorCode, String appid,
        ...
        if (errorCode == 0) {
            ...
            //如果是有插件的bind请求，就回调
            BPush plugin = (BPush)MainActivity.instance.getPlugin("BPush");
            if(plugin.currentCallbackContext == null){
                return;
            }
            JSONObject message = new JSONObject();
            try {
                message.put("appid", appid);
                message.put("channelId", userId);
                message.put("userId", userId);
                message.put("deviceToken", userId);
            }catch (JSONException e) {
            }
            Log.d(TAG, "bpush pluglin send callback:" + message.toString());
            plugin.currentCallbackContext.success(message);
            plugin.currentCallbackContext = null;
        }
    }
